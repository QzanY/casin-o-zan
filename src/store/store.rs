...
{
    let ctx = context.map_err(|_| ServerError::OtherError)?;
    ...
    match result
    {
        Ok(user) => {
            ...
            let val: Vec<(Item, i32)> = user.items.unwrap().to_string()
                .split(",")
                .map(|item| (Item::from_str(item).unwrap_or(Item::None), 0))
                .collect();
            Ok(Json(val))
        },
        Err(_) => {
...
{
    let mut ctx = context.map_err(|_| ServerError::OtherError)?;
    let result = sqlx::query!(
        "
        SELECT balance
        FROM public.users
